...
# This creates a persistent connection pool and automatic retry logic
session = requests.Session()

# Configure Retries: It will wait exponentially (2s, 4s, 8s...) if it hits a 429 or 5xx error
...
def fetch_url(url, is_json=True, is_gzipped=False, headers=None):
    ...
    # Dynamic IP spoofing to prevent IP-based rate limiting
    headers['X-Forwarded-For'] = f"64.233.{random.randint(160, 191)}.{random.randint(1, 254)}"
    
    logging.info(f"Fetching: {url}")
    try:
        # Using the session object instead of requests.get()
        ...
        if is_gzipped:
            try:
                ...
                pass # Fallback if data was already decompressed by the session
    ...
    for region in list(data['regions'].keys()) + ['all']:
        ...
        if channels:
            for c_id, ch in sorted(channels.items(), key=lambda x: x[1].get('name', '')):
                ...
                url = f'https://service-stitcher.clusters.pluto.tv/stitch/hls/channel/{ch["original_id"]}/master.m3u8?advertisingId=&appName=web&deviceMake=Chrome&deviceModel=web&deviceType=web&sid={uuid.uuid4()}&serverSideAds=true\n'
                output_lines.extend([extinf, url])
            write_m3u_file(f"plutotv_{region}.m3u", "".join(output_lines))
        time.sleep(1)

def generate_plex_m3u():
    """Plex generation without the retired anonymous token endpoint."""
    headers = {
    ...
    for region in list(found_regions) + ['all']:
        ...
        for c_id, ch in data['channels'].items():
            if region == 'all' or region in ch.get('regions', []):
                # Using jmp2 proxy with Client ID to avoid token requirement while maintaining uniqueness
        ...
        time.sleep(2) # Extra delay for Plex to avoid rapid GitHub hits
        ...
        if region == 'all':
            for r_code, r_info in data['regions'].items():
                for c_id, c_info in r_info.get('channels', {}).items(): 
                    target[f"{c_id}-{r_code}"] = {**c_info, 'original_id': c_id}
        else:
